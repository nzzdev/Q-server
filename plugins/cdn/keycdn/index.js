...
module.exports = {
  ...
  register: async function(server, options) {
    Hoek.assert(
      typeof options.zoneId === "string",
      "options.zoneId must be a string"
    );
    Hoek.assert(
      typeof options.apiKey === "string",
      "options.apiKey must be a string"
    );
    ...
    server.ext("onPreResponse", async (request, h) => {
      ...
      if (
        !request.headers.hasOwnProperty("x-pull") ||
        request.headers["x-pull"] !== options.xPullKey
      ) {
        return h.continue;
      }
    ...
    server.events.on("item.update", item => {
      const tagsToPurge = [getCacheTagForId(item._id)];
      ...
      keycdn.del(
        `zones/purgeurl/${options.zoneId}.json`,
        { tags: tagsToPurge },
        (err, res) => {
          if (err) {
